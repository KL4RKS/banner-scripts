/* 
***** PURPOSE *****
  Report to identify Highly Paid Individuals (HPIs) with possible excess pre-tax contributions 
  ...
  The first few CTEs provide a way to provide setup for the report.
    -- LIMITS_AND_WAGES - establish the prior year FICA threshold for HPI status and the standard contribution limits for each bucket by contribution year
    -- SPECIAL_CATCHUP - (optional) provide special catchup amounts for HPIs age 50+
    ...
    -- HIGHLY_PAID_50_PLUS - identifies HPIs who are age 50+
...
***** PARAMETERS *****
  This report mignt use the following parameters:
    -- Used in LIMITS_AND_WAGES and HIGHLY_PAID_50_PLUS.
...
WITH LIMITS_AND_WAGES AS -- ESTABLISH STANDARD CONTRIBUTION LIMITS AND PRIOR YEAR FICA THRESHOLD FOR HPI STATUS 
...
, SPECIAL_CATCHUP AS -- IN LIEU OF A TABLE, ADD THE SPECIAL CATCHUP (ONLY) AMOUNTS FOR MAIN (403B) AND 457B SPECIAL CATCHUP CONTRIBUTIONS (OR JUST BE AWARE OF THEM WHEN REVIEWING THE REPORT)
  -- RECORDS HERE ARE OPTIONAL. PROBABLY ONLY ADD RECORDS HERE IF YOU'VE IDENTIFIED AN EMPLOYEE WHO IS AN HPI AND IS ELIGIBLE FOR SPECIAL CATCHUP CONTRIBUTIONS AND YOU'VE CALCULATED THE AMOUNTS.
...
, HIGHLY_PAID_50_PLUS AS  -- QUERY FOR HIGHLY_PAID_50_PLUS. COULD USE BANNER FICA WAGES OR CREATE YOUR OWN LIST OF PIDMS AND INFO
  (
    SELECT 
      COALESCE(TO_CHAR(PHRHIST_POSTING_OVERRIDE_DATE, 'YYYY'), PHRDEDN_YEAR) + 1 HPI_CONTRIBUTION_YEAR
    ...
    HAVING SUM(PHRDEDN_APPLICABLE_GROSS) >= LW.PY_WAGE_THRESHOLD  -- HPI THRESHOLD
  )
...
SELECT HPI.HPI_CONTRIBUTION_YEAR CONTRIBUTION_YEAR, C.POSTING_YEAR, HPI.EE_PIDM, HPI.EE_ID, HPI.PRIOR_YEAR_FICA_RET_WAGES PY_SS_WAGES, HPI.EE_TAX_AGE
, HPI.SCL_MAIN
, SCU.SCU_MAIN SPECIAL_CATCHUP_MAIN
, HPI.SCL_MAIN + COALESCE(SCU.SCU_MAIN,0) AS PRET_ALLOWED_MAIN
...
, (HPI.SCL_MAIN + COALESCE(SCU.SCU_MAIN,0)) - SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_MAIN) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) REMAINING_PRETAX_MAIN
-- COULD REMOVE GREATEST(...,0) IF YOU WANT TO SEE NEGATIVE VALUES WHERE EMPLOYEE IS APPORACHING THE LIMIT
, GREATEST(SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_MAIN) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) - (HPI.SCL_MAIN + COALESCE(SCU.SCU_MAIN,0)),0) EXCESS_PRETAX_MAIN
...
, HPI.SCL_457B + COALESCE(SCU.SCU_457B,0) AS PRET_ALLOWED_457B
-- The next two columns show REMAINING and EXCESS amounts for pre-tax contributions in the 457B bucket.
-- You could use only one or both depending on your preference.
, (HPI.SCL_457B + COALESCE(SCU.SCU_457B,0)) - SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_457B) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) REMAINING_PRETAX_457B
-- COULD REMOVE GREATEST(...,0) IF YOU WANT TO SEE NEGATIVE VALUES WHERE EMPLOYEE IS APPORACHING THE LIMIT
, GREATEST(SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_457B) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) - (HPI.SCL_457B + COALESCE(SCU.SCU_457B,0)),0) EXCESS_PRETAX_457B
...
FROM HIGHLY_PAID_50_PLUS HPI
JOIN CONTRIBUTIONS C 
  ON  HPI.EE_PIDM = C.PHRDEDN_PIDM
  AND HPI.HPI_CONTRIBUTION_YEAR = C.POSTING_YEAR
LEFT JOIN SPECIAL_CATCHUP SCU
  ON HPI.EE_PIDM = SCU.SCU_PIDM
  AND C.POSTING_YEAR = SCU.SCU_YEAR
GROUP BY HPI.HPI_CONTRIBUTION_YEAR, C.POSTING_YEAR, HPI.EE_PIDM, HPI.EE_ID, HPI.PRIOR_YEAR_FICA_RET_WAGES, HPI.EE_TAX_AGE, HPI.SCL_MAIN, HPI.SCL_457B, SCU.SCU_MAIN, SCU.SCU_457B
-- HAVING: ONLY SHOW IF AN EMPLOYEE'S TOTAL PRE-TAX CONTRIBUTIONS (IN EITHER BUCKET) EXCEED THE STANDARD CONTRIBUTION LIMIT
HAVING ( SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_MAIN) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) > (HPI.SCL_MAIN + COALESCE(SCU.SCU_MAIN,0))
      OR SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_457B) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) > (HPI.SCL_457B + COALESCE(SCU.SCU_457B,0))
       )
...
-- HAVING ( SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_MAIN) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) > (HPI.SCL_MAIN + COALESCE(SCU.SCU_MAIN,0) - COALESCE(TO_NUMBER(:MAIN_EB_WITHIN_AMOUNT),0))
--       OR SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_457B) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) > (HPI.SCL_457B + COALESCE(SCU.SCU_457B,0) - COALESCE(TO_NUMBER(:MAIN_EB_WITHIN_AMOUNT),0))
--        )
;