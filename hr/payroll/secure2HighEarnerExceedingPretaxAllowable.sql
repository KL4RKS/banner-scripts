...
, HIGH_EARNERS_50_PLUS AS  -- QUERY FOR HIGH_EARNERS_50_PLUS. COULD USE BANNER FICA WAGES OR CREATE YOUR OWN LIST OF PIDMS AND INFO
  (
    SELECT 
      PHRDEDN_YEAR + 1 HE_CONTRIBUTION_YEAR
...
, CONTRIBUTIONS AS -- QUERY FOR ALL EMPLOYEE CONTRIBUTION AMOUNTS IN ALL BUCKETS FOR THE CURRENT YEAR
  (
    SELECT 
      ...
      -- TO_CHAR(PHRHIST_POSTING_OVERRIDE_DATE, 'YYYY') PHRDEDN_YEAR
      PHRDEDN_YEAR
    , PHRDEDN_PIDM
    , PHRDEDN_BDCA_CODE
    ...
    WHERE PHRDEDN_BDCA_CODE IN 
      ...
      -- -- 2025-10-24: I COULDN'T GET THIS WORKING, IT WAS RETURNING 0 RECORDS.
      -- AND TO_CHAR(PHRHIST_POSTING_OVERRIDE_DATE, 'YYYY') = TO_CHAR(:CONTRIBUTION_YEAR) -- TO_CHAR(SYSDATE + 5, 'YYYY')
    GROUP BY 
      PHRDEDN_YEAR
      -- -- 2025-10-24: I COULDN'T GET THIS WORKING, IT WAS RETURNING 0 RECORDS.
      -- PHRHIST_POSTING_OVERRIDE_DATE
    , PHRDEDN_PIDM
    , PHRDEDN_BDCA_CODE
  )
...
SELECT HE.HE_CONTRIBUTION_YEAR CONTRIBUTION_YEAR, HE.EE_PIDM, HE.EE_ID, HE.PRIOR_YEAR_FICA_RET_WAGES PY_SS_WAGES, HE.EE_TAX_AGE
, LW.NL_415
, SC.SC_415 SPECIAL_CATCHUP_415
, LW.NL_415 + COALESCE(SC.SC_415,0) AS PRET_ALLOWED_415
, (LW.NL_415 + COALESCE(SC.SC_415,0)) - SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_415) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) REMAINING_PRETAX_415
-- COULD REMOVE GREATEST(...,0) IF YOU WANT TO SEE NEGATIVE VALUES WHERE EMPLOYEE IS APPORACHING THE LIMIT
, GREATEST(SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_415) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) - (LW.NL_415 + COALESCE(SC.SC_415,0)),0) EXCESS_PRETAX_415
, SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_415) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) AS PRET_415_TOTAL
, SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM POST_415) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) AS POST_415_TOTAL
, SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_415 UNION SELECT PTRBDCA_CODE FROM POST_415) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) AS TOTAL_415
, LW.NL_457
, SC.SC_457 SPECIAL_CATCHUP_457
, LW.NL_457 + COALESCE(SC.SC_457,0) AS PRET_ALLOWED_457
, (LW.NL_457 + COALESCE(SC.SC_457,0)) - SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_457) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) REMAINING_PRETAX_457
-- COULD REMOVE GREATEST(...,0) IF YOU WANT TO SEE NEGATIVE VALUES WHERE EMPLOYEE IS APPORACHING THE LIMIT
, GREATEST(SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_457) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) - (LW.NL_457 + COALESCE(SC.SC_457,0)),0) EXCESS_PRETAX_457
, SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_457) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) AS PRET_457_TOTAL
, SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM POST_457) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) AS POST_457_TOTAL
, SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_457 UNION SELECT PTRBDCA_CODE FROM POST_457) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) AS TOTAL_457
...
JOIN CONTRIBUTIONS C 
  ...
  AND HE.HE_CONTRIBUTION_YEAR = C.PHRDEDN_YEAR
JOIN LIMITS_AND_WAGES LW ON LW.LW_CONTRIBUTION_YEAR = HE.HE_CONTRIBUTION_YEAR
LEFT JOIN SPECIAL_CATCHUP SC
  ON HE.EE_PIDM = SC.SC_PIDM
  AND C.PHRDEDN_YEAR = SC.SC_YEAR
GROUP BY HE.HE_CONTRIBUTION_YEAR, HE.EE_PIDM, HE.EE_ID, HE.PRIOR_YEAR_FICA_RET_WAGES, HE.EE_TAX_AGE, LW.NL_415, LW.NL_457, SC.SC_415, SC.SC_457
...
HAVING ( SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_415) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) > (LW.NL_415 + COALESCE(SC.SC_415,0) - COALESCE(TO_NUMBER(:WITHIN_AMOUNT),0))
      OR SUM (CASE WHEN C.PHRDEDN_BDCA_CODE IN (SELECT PTRBDCA_CODE FROM PRET_457) THEN C.TOTAL_EE_CONTRIBUTIONS ELSE 0 END) > (LW.NL_457 + COALESCE(SC.SC_457,0) - COALESCE(TO_NUMBER(:WITHIN_AMOUNT),0))
       )
;